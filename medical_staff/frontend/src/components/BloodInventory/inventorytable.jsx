...
export default function InventoryTable1() {
  ...
  const fetchInventory = async (pageNum = 1, append = false) => {
    try {
      ...
      // If backend returns array, estimate totalPages
      else if (Array.isArray(data)) {
        setTotalPages(pageNum + (data.length === 10 ? 1 : 0));
      }
      if (append) {
        setInventoryData(prev => [...prev, ...(data.data || data)])
      } else {
        setInventoryData(data.data || data)
      }
      setHasMore((data.data ? data.data.length : data.length) === 10 && pageNum < (data.meta ? data.meta.totalPages : pageNum + 1))
      setLoading(false)
