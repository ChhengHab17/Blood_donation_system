...
export default function CreateDonorAccount() {
  ...
  const [formData, setFormData] = useState({
    ...
    phone_num: "",
    email: "",
    last_donation_date: getTodayDateString(),
    volume: 450,
    is_eligible: true,
    hemoglobin_level: "",
    blood_pressure: "",
  ...
  const validateForm = () => {
    ...
    if (formData.is_eligible) {
      if (!formData.volume || Number(formData.volume) <= 0) newErrors.volume = "Donation volume is required and must be greater than 0 for eligible donors";
    } else {
      if (formData.volume !== 0 && formData.volume !== "0") newErrors.volume = "Donation volume must be 0 for ineligible donors";
    }
    if (formData.hemoglobin_level === "" || isNaN(Number(formData.hemoglobin_level))) newErrors.hemoglobin_level = "Hemoglobin level is required and must be a number"
    if (!formData.blood_pressure) newErrors.blood_pressure = "Blood pressure is required"
    if (formData.weight === "" || isNaN(Number(formData.weight))) newErrors.weight = "Weight is required and must be a number"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        volume: formData.is_eligible ? Number(formData.volume) : 0,
        is_eligible: Boolean(formData.is_eligible),
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <button onClick={handleCancel} className="text-blue-600 hover:text-blue-700 flex items-center gap-2 hover:cursor-pointer">
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="is_eligible" className="block text-sm font-medium text-gray-700 mb-2">
                      Eligible to Donate? *
                    </label>
                    <select
                      id="is_eligible"
                      value={formData.is_eligible ? "true" : "false"}
                      onChange={e => {
                        const eligible = e.target.value === "true";
                        handleInputChange("is_eligible", eligible);
                        if (!eligible) {
                          handleInputChange("volume", 0);
                        } else if (formData.volume === 0 || formData.volume === "0") {
                          handleInputChange("volume", 450);
                        }
                      }}
                      className="w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 ${errors.hemoglobin_level ? "border-red-500" : "border-gray-300"}`}
                      placeholder="e.g. 13.5"
                    />
                    {errors.hemoglobin_level && <p className="mt-1 text-sm text-red-600">{errors.hemoglobin_level}</p>}
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 ${errors.blood_pressure ? "border-red-500" : "border-gray-300"}`}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 ${errors.weight ? "border-red-500" : "border-gray-300"}`}
                      placeholder="e.g. 65"
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Enter donation volume (default 450)"
                      disabled={!formData.is_eligible}
                    />
