...
export const getAllBloodRequestsCon = async (req, res) => {
  try {
    ...
    const result = await getAllBloodRequests(page, limit, status, requestType, search, center_id);
...
export const createBloodRequestCon = async (req, res) => {
  try {
    ...
    const center_id = req.user.center_id;
    const bloodRequest = await createBloodRequest({ ...bloodRequestData, center_id });
